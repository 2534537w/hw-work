...
FIR_filter::FIR_filter( int taps, double f1, double f2, fir_type type,
                        window_type window): h(taps, 0), samples(taps, 0)
{
    this->idx     = 0;
    this->taps    = taps;
    ...
    switch (type) {
        ...
        case fir_type_bandpass:
            ...
            break;
        case fir_type_bandstop:
            h = bandStop_coefficient(taps, f1, f2);
            break;
        default:
            throw std::invalid_argument("not support fir type\n");
    }
