...
public class Snap extends CardGame {
    ...
    protected boolean isSnap(Card previousCard, Card currentCard, boolean snapOnSuit) {
        if (!snapOnSuit) {
            if (previousCard.getCardRank().ordinal() == currentCard.getCardRank().ordinal())
                return true;
            if (previousCard.getCardRank().ordinal() != currentCard.getCardRank().ordinal())
                return false;

            if (previousCard.getSuit().ordinal() == currentCard.getSuit().ordinal())
                return false;
            else
                return true;
        }
        ...
        if (snapOnSuit) {
            if (previousCard.getSuit().ordinal() == currentCard.getSuit().ordinal())
                return true;
            else if((previousCard.getSuit().ordinal() != currentCard.getSuit().ordinal()))
                return false;

            if (previousCard.getCardRank().ordinal() == currentCard.getCardRank().ordinal())
                return false;
            else if (previousCard.getCardRank().ordinal() != currentCard.getCardRank().ordinal())
                return true;
        }
