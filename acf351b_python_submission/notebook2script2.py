...
def notebook2scriptSingle(fname, destination):
    ...
    fname_out = 'submission_part_II.py'
    ...
    try:
        if (len(sys.argv) > 2): # Warning if extra parameters have been provided
            print("Please note that the notebook will now be submitted to the first directory specified immediately "
                  "after \'helpers/AcF_351b_Python_Stream_Final_Exam_Part_I.ipynb\'. Any arguments that come after are ignored.\n")
        folder_location = sys.argv[1]
        os.makedirs(folder_location,exist_ok=True)
        notebook2scriptSingle('AcF_351b_Python_Stream_Final_Exam_Part_II.ipynb', folder_location)
    except IndexError:
